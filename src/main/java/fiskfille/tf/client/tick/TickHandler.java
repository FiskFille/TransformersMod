...
public class TickHandler
{
    ...
    public void onKeyInput(KeyInputEvent event)
    {
        ...
        if (TFKeyBinds.keyBindingTransform.getIsKeyPressed()
                ...
                && player.ridingEntity == null)
        {
            ...
            if (inVehicleMode && transformationTimer == 0)
            {
                ...
                player.playSound(TransformersMod.modid + ":transform_robot",
                        1.0F, 1.0F);
            ...
            else if (!inVehicleMode && transformationTimer == 20)
            {
                ...
                player.playSound(TransformersMod.modid + ":transform_vehicle",
                        1.0F, 1.0F);
                TFPlayerData.getData(player).stealthForce = false;
            ...
            try
            {
                float camRoll = ClientProxy.camRollField
                        .getFloat(entityRenderer);
        ...
        if (TFKeyBinds.keyBindingStealthMode.getIsKeyPressed())
        {
            ...
            if (transformer != null)
            {
                if (inVehicleMode && mc.currentScreen == null
                        && transformer.hasStealthForce(player))
                {
                    int stealthModeTimer = TFDataManager
                            .getStealthModeTimer(player);

                    if (TFDataManager.isInStealthMode(player)
                            && stealthModeTimer == 0)
                    {
                        TFDataManager.setInStealthMode(player, false);
                        player.playSound(TransformersMod.modid
                                + ":transform_robot", 1.0F, 1.5F);
                    }
                    else if (!TFDataManager.isInStealthMode(player)
                            && stealthModeTimer == 5)
                    {
                        TFDataManager.setInStealthMode(player, true);
                        player.playSound(TransformersMod.modid
                                + ":transform_vehicle", 1.0F, 1.5F);
                    }
                }
            }
        }
    ...
    public void onPlayerTick(PlayerTickEvent event)
    {
        ...
        if (TFDataManager.getZoomTimer(player) > 7)
        {
            ...
            if (heldItem != null
                    && heldItem.getItem() instanceof ItemVurpsSniper)
            {
