...
public class ClientTickHandler
{
    ...
    public void onPlayerTick(EntityPlayer player)
    {
        ...
        if (TFHelper.isPlayerVurp(player) && heldItem != null
                && heldItem.getItem() == TFItems.vurpsSniper
                && TFKeyBinds.keyBindingZoom.getIsKeyPressed())
        {
            if (TFDataManager.getZoomTimer(player) < 10)
            {
                TFDataManager.setZoomTimer(player,
                        TFDataManager.getZoomTimer(player) + 1);
            }
        }
        else
        {
        ...
        if (inVehicleMode && transformationTimer < 10)
        {
            ...
            if (player == mc.thePlayer)
            {
                ...
                else if (TFKeyBinds.keyBindingVehicleFirstPerson
                        .getIsKeyPressed())
                {
        ...
        else
        {
            if (transformer != null)
            {
                if (transformer.hasJetpack())
                {
                    ...
                    boolean jetpacking = mc.gameSettings.keyBindJump
                            .getIsKeyPressed();
                    ...
                    if (isClientPlayer)
                    {
                        if (prevJetpacking != jetpacking)
                        {
                            TFPacketManager.networkWrapper
                                    .sendToServer(new PacketCloudtrapJetpack(
                                            player, jetpacking));
                        ...
                        if (cloudtrapJetpacking.get(player))
                        {
                            for (int i = 0; i < 20; ++i)
                            {
                                ...
                                player.worldObj.spawnParticle("flame",
                                        ...
                                        rand.nextFloat() / 4 - 0.125F, -0.8F,
                                        rand.nextFloat() / 4 - 0.125F);
                            }
                        }
                    }
            ...
            if (player.isPotionActive(Potion.resistance)
                    && player.getActivePotionEffect(Potion.resistance)
                            .getDuration() < 1)
            {
        ...
        int nitro = transformedPlayer == null ? 0 : transformedPlayer
                .getNitro();
        ...
        if ((nitro < 160 && (player.capabilities.isCreativeMode || !((nitroPressed && !TFDataManager
                .isInStealthMode(player)) && moveForward && inVehicleMode && transformationTimer < 10))))
        {
            ++nitro;
        }
    ...
    public void handleTransformation(EntityPlayer player)
    {
        ...
        float offsetY = getCameraOffset(player, transformer)
                + (float) transformationTimer / 20;
        ...
        if (transformationTimer < 20 && !inVehicleMode)
        {
            ...
            if (transformationTimer == 19)
            {
                ...
                {
                    mc.gameSettings.thirdPersonView = TFConfig.firstPersonAfterTransformation ? 0
    ...
    public void onTickEnd()
    {
        ...
        try
        {
            ...
            float thirdPersonDistance = 4.0F - (2.0F - (float) TFDataManager
                    .getTransformationTimer(player) / 10);

            if (transformer != null && (transformer.canZoom(player))
            ...
            {
                thirdPersonDistance = transformer
                        .getThirdPersonDistance(player);
            }

            ObfuscationReflectionHelper.setPrivateValue(EntityRenderer.class,
                    mc.entityRenderer, thirdPersonDistance, new String[] {
