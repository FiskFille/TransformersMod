...
public class MowzieModelBase extends ModelChildBase.Biped
{
    ...
    /**
     * Resets the rotate angles and rotation points to its original value if
     * they were saved before. Note: Call this at the beginning of
     * setRotationAngles.
     * 
    ...
    protected void addChildTo(ModelRenderer child, ModelRenderer parent)
    {
        float distance = (float) Math.sqrt(Math.pow(
                (child.rotationPointZ - parent.rotationPointZ), 2)
                + Math.pow((child.rotationPointY - parent.rotationPointY), 2));
        float oldRotateAngleX = parent.rotateAngleX;
        float parentToChildAngle = (float) Math
                .atan((child.rotationPointZ - parent.rotationPointZ)
                        / (child.rotationPointY - parent.rotationPointY));
        float childRelativeRotation = parentToChildAngle - parent.rotateAngleX;
        float newRotationPointY = (float) (distance * (Math
                .cos(childRelativeRotation)));
        float newRotationPointZ = (float) (distance * (Math
                .sin(childRelativeRotation)));
        parent.rotateAngleX = 0F;
        child.setRotationPoint(child.rotationPointX - parent.rotationPointX,
                newRotationPointY, newRotationPointZ);
    ...
    /**
     * Don't use this yet. I'm trying to refine the parenting method, but it's
     * not ready yet.
     */
    protected void newAddChildTo(ModelRenderer child, ModelRenderer parent)
    {
        float distance = (float) Math.sqrt(Math.pow(
                (child.rotationPointZ - parent.rotationPointZ), 2)
                + Math.pow((child.rotationPointY - parent.rotationPointY), 2));
        float angle = (float) Math.atan2(child.rotationPointY
                - parent.rotationPointY, child.rotationPointZ
                - parent.rotationPointZ);
     ...
     * 
     * @param f
     *            is the number of boxes being used. (i.e. if you are using this
     *            on a head and neck, set it to 2. Just a head, 1);
     * @param f3
     ...
     * @param weight
     *            will make the animation favor one direction more based on how
     *            fast the mob is moving;
     * @param f
     *            is the walked distance;
     * @param f1
     *            is the walk speed.
     */
    public float rotateBox(float speed, float degree, boolean invert,
            float offset, float weight, float f, float f1)
    {
        if (invert)
            return -MathHelper.cos(f * speed + offset) * degree * f1 + weight
                    * f1;
        else
            return MathHelper.cos(f * speed + offset) * degree * f1 + weight
                    * f1;
     ...
     *            is the walk speed.
     */
    public float moveBox(float speed, float degree, boolean bounce, float f,
            float f1)
    {
        if (bounce)
     ...
     *            is the walk speed.
     */
    public void walk(MowzieModelRenderer box, float speed, float degree,
            boolean invert, float offset, float weight, float f, float f1)
    {
        int inverted = 1;
        if (invert)
            inverted = -1;
        box.rotateAngleX += MathHelper.cos(f * speed + offset) * degree
                * inverted * f1 + weight * f1;
    }
     ...
     *            is the walk speed.
     */
    public void flap(MowzieModelRenderer box, float speed, float degree,
            boolean invert, float offset, float weight, float f, float f1)
    {
        int inverted = 1;
        if (invert)
            inverted = -1;
        box.rotateAngleZ += MathHelper.cos(f * speed + offset) * degree
                * inverted * f1 + weight * f1;
    }
     ...
     *            is the walk speed.
     */
    public void swing(MowzieModelRenderer box, float speed, float degree,
            boolean invert, float offset, float weight, float f, float f1)
    {
        int inverted = 1;
        if (invert)
            inverted = -1;
        box.rotateAngleY += MathHelper.cos(f * speed + offset) * degree
                * inverted * f1 + weight * f1;
    }
     ...
     *            is the walk speed.
     */
    public void bob(MowzieModelRenderer box, float speed, float degree,
            boolean bounce, float f, float f1)
    {
    ...
    /**
     * Swings a chain of parented boxes back and forth (rotateAngleY). Useful
     * for tails.
     * <p/>
     * Note: Just keep f and f1 from the setRotationAngles() method.
     * 
     ...
     *            is the walk speed.
     */
    public void chainSwing(MowzieModelRenderer[] boxes, float speed,
            float degree, double rootOffset, float f, float f1)
    {
        int numberOfSegments = boxes.length;
        float offset = (float) ((rootOffset * Math.PI) / (2 * numberOfSegments));
        for (int i = 0; i < numberOfSegments; i++)
            boxes[i].rotateAngleY += MathHelper.cos(f * speed + offset * i)
                    * f1 * degree;
    }

    /**
     * Swings a chain of parented boxes up and down (rotateAngleX). Useful for
     * tails.
     * <p/>
     * Note: Just keep f and f1 from the setRotationAngles() method.
     * 
     ...
     *            is the walk speed.
     */
    public void chainWave(MowzieModelRenderer[] boxes, float speed,
            float degree, double rootOffset, float f, float f1)
    {
        int numberOfSegments = boxes.length;
        float offset = (float) ((rootOffset * Math.PI) / (2 * numberOfSegments));
        for (int i = 0; i < numberOfSegments; i++)
            boxes[i].rotateAngleX += MathHelper.cos(f * speed + offset * i)
                    * f1 * degree;
    }

    /**
     * Flaps a chain of parented boxes up and down (rotateAngleZ). Useful for
     * tails.
     * <p/>
     * Note: Just keep f and f1 from the setRotationAngles() method.
     * 
     ...
     *            is the walk speed.
     */
    public void chainFlap(MowzieModelRenderer[] boxes, float speed,
            float degree, double rootOffset, float f, float f1)
    {
        int numberOfSegments = boxes.length;
        float offset = (float) ((rootOffset * Math.PI) / (2 * numberOfSegments));
        for (int i = 0; i < numberOfSegments; i++)
            boxes[i].rotateAngleZ += MathHelper.cos(f * speed + offset * i)
                    * f1 * degree;
    }
    ...
    public boolean onGround(Entity entity)
    {
        if (entity != Minecraft.getMinecraft().thePlayer)
        {
            ...
            List collidingEntities = entity.worldObj.getCollidingBoundingBoxes(
                    entity, entity.boundingBox.addCoord(0, moveY, 0));

            for (int currentIndex = 0; currentIndex < collidingEntities.size(); ++currentIndex)
            {
                moveY = ((AxisAlignedBB) collidingEntities.get(currentIndex))
                        .calculateYOffset(entity.boundingBox, moveY);
            }
