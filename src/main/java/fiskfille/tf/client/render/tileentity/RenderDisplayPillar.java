...
public class RenderDisplayPillar extends TileEntitySpecialRenderer
{
    private ResourceLocation texture = new ResourceLocation(
            TransformersMod.modid + ":textures/models/tiles/display_pillar.png");
    ...
    public void renderModelAt(TileEntityDisplayPillar tileentity, double d,
            double d1, double d2, float f)
    {
        ...
        if (displayItem != null)
        {
            this.bindTexture(new ResourceLocation(TransformersMod.modid,
                    "textures/models/" + getTextureFromStack(displayItem)));
            ModelVehicleBase vehicle = getModelFromStack(displayItem);

            if (vehicle != null)
            {
                GL11.glRotatef(Minecraft.getMinecraft().thePlayer.ticksExisted,
                        0.0F, 1.0F, 0.0F);
    ...
    {
        Transformer transformer = TransformersAPI.getTransformers().get(
                displayItem.getItemDamage());
    ...
    public String getTextureFromStack(ItemStack displayItem)
    {
        ...
        if (transformer != null)
        {
            String name = transformer.getName().toLowerCase()
                    .replaceAll(" ", "_");
