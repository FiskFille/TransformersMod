...
public class RenderCustomPlayer extends RenderPlayer
{
    ...
    protected void renderEquippedItems(AbstractClientPlayer player,
            float partialTicks)
    {
        ...
        if (helmetStack != null && event.renderHelmet)
        {
            ...
            if (helmet instanceof ItemBlock)
            {
                ...
                this.renderManager.itemRenderer.renderItem(player, helmetStack,
                        0);
            }
            else if (helmet == Items.skull)
            {
                ...
                if (helmetStack.hasTagCompound())
                {
                    ...
                    {
                        gameprofile = NBTUtil.func_152459_a(itemTag
                                .getCompoundTag("SkullOwner"));
                    }
                    else if (itemTag.hasKey("SkullOwner", 8)
                            && !StringUtils.isNullOrEmpty(itemTag
                                    .getString("SkullOwner")))
                    {
                        gameprofile = new GameProfile((UUID) null,
                                itemTag.getString("SkullOwner"));
                    }
                }

                TileEntitySkullRenderer.field_147536_b.func_152674_a(-0.5F,
                        0.0F, -0.5F, 1, 180.0F, helmetStack.getItemDamage(),
                        gameprofile);
            }
        ...
        if (player.getCommandSenderName().equals("deadmau5") && hasSkin)
        {
            ...
            for (int j = 0; j < 2; ++j)
            {
                float f9 = player.prevRotationYaw
                        + (player.rotationYaw - player.prevRotationYaw)
                        * partialTicks
                        - (player.prevRenderYawOffset + (player.renderYawOffset - player.prevRenderYawOffset)
                                * partialTicks);
                float f10 = player.prevRotationPitch
        ...
        if (hasCape && !player.isInvisible() && !player.getHideCape())
        {
            ...
            if (transformer instanceof TransformerPurge) // TODO some sort of
                                                         // api for this
            {
                ModelPurge purge = (ModelPurge) TFModelRegistry
            ...
            {
                ModelSkystrike skystrike = (ModelSkystrike) TFModelRegistry
                        .getModel(transformer);
            ...
            {
                ModelSubwoofer subwoofer = (ModelSubwoofer) TFModelRegistry
                        .getModel(transformer);
            ...
            {
                ModelVurp vurp = (ModelVurp) TFModelRegistry
                        .getModel(transformer);
            ...
            double d2 = (double) (-MathHelper
                    .cos(f4 * (float) Math.PI / 180.0F));
            ...
            float f8 = player.prevCameraYaw
                    + (player.cameraYaw - player.prevCameraYaw) * partialTicks;
            f5 += MathHelper
                    .sin((player.prevDistanceWalkedModified + (player.distanceWalkedModified - player.prevDistanceWalkedModified)
                            * partialTicks) * 6.0F)
                    * 32.0F * f8;
        ...
        if (heldItemStack != null && event.renderItem)
        {
            ...
            {
                ModelCloudtrap cloudtrap = (ModelCloudtrap) TFModelRegistry
                        .getModel(transformer);
            ...
            if (is3D
                    || heldItem instanceof ItemBlock
                    && RenderBlocks.renderItemIn3d(Block.getBlockFromItem(
                            heldItem).getRenderType()))
            {
                f2 = 0.5F;
            ...
            {
                for (k = 0; k < heldItem.getRenderPasses(heldItemStack
                        .getItemDamage()); ++k)
                {
                    ...
                    this.renderManager.itemRenderer.renderItem(player,
                            heldItemStack, k);
                }
            }
            else
        ...
        net.minecraftforge.common.MinecraftForge.EVENT_BUS
                .post(new net.minecraftforge.client.event.RenderPlayerEvent.Specials.Post(
                        player, this, partialTicks));
    }
    ...
    public void renderFirstPersonArm(EntityPlayer player)
    {
        ...
        if (!TFDataManager.isInVehicleMode(player))
        {
            if (currentArmor != null)
            {
                ...
                {
                    // ITransformerArmor transformerArmor =
                    // (ITransformerArmor)currentArmor.getItem();
                    // Transformer transformer =
                    // transformerArmor.getTransformer();
                    //
                    // if(transformer != null)
                    // {
                    // ModelChildBase.Biped model = transformer.getModel();
                    // ResourceLocation resourcelocation = new
                    // ResourceLocation(transformer.getChestplate().getArmorTexture(currentArmor,
                    // player, 3, ""));
                    //
                    // float f = 1.0F;
                    ...
                    // model.onGround = 0.0F;
                    // model.setRotationAngles(0.0F, 0.0F, 0.0F, 0.0F, 0.0F,
                    // 0.0625F, player);
