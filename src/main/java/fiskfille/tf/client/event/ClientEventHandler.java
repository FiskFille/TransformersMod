...
public class ClientEventHandler
{
    ...
    public void onPlaySound(PlaySoundAtEntityEvent event)
    {
        if (event.entity instanceof EntityPlayer)
        {
            if (event.name.startsWith("step.")
                    && TFDataManager
                            .isInVehicleMode((EntityPlayer) event.entity))
            {
    ...
    public void onRenderPlayerPre(RenderPlayerEvent.Pre event)
    {
        ...
        // This prevents the player from sinking into the ground when sneaking
        // in vehicle mode
    ...
    public void onRenderPlayer(RenderPlayerEvent.Post event)
    {
        ...
        // ObfuscationReflectionHelper.getPrivateValue(RenderPlayer.class,
        // event.renderer, new String[]{"f", "modelBipedMain"});
        // TFModelHelper.modelBipedMain = modelBipedMain;
        //
        // ObfuscationReflectionHelper.setPrivateValue(RenderPlayer.class,
        // event.renderer, modelBipedMain, new String[]{"f", "modelBipedMain"});
    }
    ...
    public void onFOVUpdate(FOVUpdateEvent event)
    {
        ...
        int nitro = transformedPlayer == null ? 0 : transformedPlayer
                .getNitro();
        boolean moveForward = Minecraft.getMinecraft().gameSettings.keyBindForward
                .getIsKeyPressed();
        ...
        {
            if (nitro > 0 && moveForward && nitroPressed
        ...
        else
        {
            ...
            if (TFDataManager.getZoomTimer(player) > 0
                    && TFHelper.isPlayerVurp(player) && itemstack != null
                    && itemstack.getItem() == TFItems.vurpsSniper
                    && this.mc.gameSettings.thirdPersonView == 0)
            {
                event.newfov = 1.0F - (float) TFDataManager
                        .getZoomTimer(player) / 10;
            }
        }

        if (TFDataManager.getTransformationTimer(player) < 20
                && !(nitro > 0 && moveForward && nitroPressed && !TFDataManager
                        .isInStealthMode(player)))
        {
            event.newfov = 1.0F;
        }
    }
