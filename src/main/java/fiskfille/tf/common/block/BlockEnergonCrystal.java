...
public class BlockEnergonCrystal extends BlockBasic implements
        ITileEntityProvider
{
    ...
    public boolean canPlaceBlockAt(World world, int x, int y, int z)
    {
        return world.isSideSolid(x - 1, y, z, EAST, true)
                ...
                || isSolid(world, x, y - 1, z) || isSolid(world, x, y + 1, z);
    }
    ...
    protected boolean neighbourChanged(World world, int x, int y, int z,
            Block block)
    {
                ...
                if (new Random().nextInt(9) == 0)
                {
                    if (!world.isRemote
                            && world.getGameRules().getGameRuleBooleanValue(
                                    "doTileDrops")
                            && !world.restoringBlockSnapshots) // do not drop
                                                               // items while
                                                               // restoring
                                                               // blockstates,
                                                               // prevents item
                    ...
                    {
                        float f = 0.7F;
                        double motionX = (double) (world.rand.nextFloat() * f)
                                + (double) (1.0F - f) * 0.5D;
                        double motionY = (double) (world.rand.nextFloat() * f)
                                + (double) (1.0F - f) * 0.5D;
                        double motionZ = (double) (world.rand.nextFloat() * f)
                                + (double) (1.0F - f) * 0.5D;
                        EntityItem entityitem = new EntityItem(world,
                                (double) x + motionX, (double) y + motionY,
                                (double) z + motionZ, new ItemStack(
                                        TFBlocks.energonCrystal));
            ...
            Vec3 p_149731_5_, Vec3 p_149731_6_)
    {
        ...
        {
            f = 0.2F;
            this.setBlockBounds(0.5F - f, 0.0F + f * 2, 0.5F - f, 0.5F + f,
                    1.0F, 0.5F + f);
        }
        else
        {
    ...
    {
        blockIcon = par1IconRegister.registerIcon(TransformersMod.modid
                + ":energon_crystal");
    }
}