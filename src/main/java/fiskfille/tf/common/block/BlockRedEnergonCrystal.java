...
public class BlockRedEnergonCrystal extends BlockBasic implements ITileEntityProvider
{
    private Random rand = new Random();
    
    public BlockRedEnergonCrystal()
    {
            ...
            if (!canSupport)
            {
                if (new Random().nextInt(9) == 0)
                {
                    if (!world.isRemote && world.getGameRules().getGameRuleBooleanValue("doTileDrops") && !world.restoringBlockSnapshots) // do not drop items while restoring blockstates, prevents item dupe
                    {
                        ...
                        EntityItem entityitem = new EntityItem(world, (double) x + motionX, (double) y + motionY, (double) z + motionZ, new ItemStack(TFBlocks.redEnergonCrystal));
    ...
    {
        blockIcon = par1IconRegister.registerIcon(TransformersMod.modid + ":red_energon_crystal");
    }
}