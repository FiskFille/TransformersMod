...
public abstract class TransformerJet extends Transformer
{
    ...
    public void updateMovement(EntityPlayer player)
    {
        ...
        boolean moveForward = minecraft.gameSettings.keyBindForward
                .getIsKeyPressed();
        boolean nitroPressed = TFKeyBinds.keyBindingNitro.getIsKeyPressed()
                || minecraft.gameSettings.keyBindSprint.getIsKeyPressed();
        ...
        if (motion != null)
        {
            ...
            if ((player.worldObj.getBlock((int) player.posX,
                    (int) player.posY - 1, (int) player.posZ) != Blocks.air
                    || player.worldObj.getBlock((int) player.posX,
                            (int) player.posY - 2, (int) player.posZ) != Blocks.air || player.worldObj
                        .getBlock((int) player.posX, (int) player.posY - 3,
                                (int) player.posZ) != Blocks.air))
            {
            ...
            if (player == Minecraft.getMinecraft().thePlayer)
            {
                if (nitro > 0 && nitroPressed && moveForward)
                {
                    ...
                    if (!prevNitro)
                    {
                        TFPacketManager.networkWrapper
                                .sendToServer(new PacketVehicleNitro(player,
                                        true));
                    ...
                    for (int i = 0; i < 4; ++i)
                    {
                        ...
                        player.worldObj.spawnParticle("flame", side.xCoord,
                                side.yCoord - 0.2F, side.zCoord,
                                rand.nextFloat() / 20, -0.2F + rand.nextFloat()
                ...
                try
                {
                    ...
                    {
                        EntityRenderer entityRenderer = Minecraft
                                .getMinecraft().entityRenderer;
    ...
    {
        return new EntityMissile(player.worldObj, player, 3,
                TFConfig.allowMissileExplosions,
    ...
    public void doNitroParticles(EntityPlayer player)
    {
        for (int i = 0; i < 4; ++i)
        {
            ...
            Random rand = new Random();
            player.worldObj.spawnParticle("flame", side.xCoord,
                    side.yCoord + 1.5F, side.zCoord, rand.nextFloat() / 20,
                    -0.2F + rand.nextFloat() / 20, rand.nextFloat() / 20);
        }
    }
