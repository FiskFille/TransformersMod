...
public abstract class TransformerCar extends Transformer
{
    ...
    {
        return TFDataManager.isInVehicleMode(player) ? distance / 2 : super
    ...
    public void updateMovement(EntityPlayer player)
    {
        ...
        if (transformedPlayer != null)
        {
            ...
            if (driftPressed && player.onGround && forwardVelocity > 0)
            {
                ...
                float f = modelBipedMain.bipedHead.rotateAngleY
                        - (modelBipedMain.bipedBody.rotateAngleY - modelBipedMain.bipedHead.rotateAngleY)
                        / 3;
                forwardVelocity -= 0.03D;

                Vec3 vec3 = TFMotionManager.getSideCoords(player,
                        forwardVelocity, -(int) (f * 45)/*
                                                         * f > -0.25D && f <
                                                         * 0.25D ? 0 : (f > 0 ?
                                                         * -45 : 30)
                                                         */);
                ...
                if (forwardVelocity > 0.1D)
                {
                    for (int i = 0; i < 10; ++i)
                    {
                        Vec3 side = TFMotionManager.getBackSideCoords(player,
                                0.15F, i < 2, -0.5F, false);
                        player.worldObj.spawnParticle("reddust", side.xCoord,
                                player.posY - 1.5F, side.zCoord, -1, 0, 0);
                    }
                }
