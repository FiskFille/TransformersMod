...
public class ItemMetaBasic extends Item
{
    ...
    public String getItemStackDisplayName(ItemStack stack)
    {
        ...
        return StatCollector
                .translateToLocal(itemNames[itemDamage > itemNames.length ? itemNames.length
                        : itemDamage]);
    }
    ...
    public void registerIcons(IIconRegister par1IconRegister)
    {
        ...
        for (int i = 0; i < itemNames.length; ++i)
        {
            icons[i] = par1IconRegister.registerIcon(TransformersMod.modid
                    + ":"
                    + itemNames[i].toLowerCase().replace(' ', '_')
                            .replace("'", ""));
        }
    }
}