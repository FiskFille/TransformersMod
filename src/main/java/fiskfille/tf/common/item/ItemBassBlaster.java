...
public class ItemBassBlaster extends ItemSword
{
        ...
        if (duration < 40)
        {
                    ...
                    || player.capabilities.isCreativeMode)
            {
                ...
                world.playSound(player.posX, player.posY, player.posZ,
                        "note.bassattack", 1.0F, 0.8F, true);
                world.playSound(player.posX, player.posY, player.posZ,
                        "note.bass", 1.0F, 0.8F, true);

                if (!world.isRemote)
                {
                    EntityBassCharge entity = new EntityBassCharge(world,
                            player);
    ...
    public void registerIcons(IIconRegister iconRegister)
    {
        itemIcon = iconRegister.registerIcon(TransformersMod.modid + ":"
                + iconString);
        bassChargeIcon = iconRegister.registerIcon(TransformersMod.modid
                + ":bass_charge");
    }
}