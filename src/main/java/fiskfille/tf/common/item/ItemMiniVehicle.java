...
public class ItemMiniVehicle extends Item implements IDisplayPillarItem
{
    ...
    public String getItemStackDisplayName(ItemStack stack)
    {
        ...
        if (transformer != null)
        {
            return StatCollector.translateToLocal("item.display_"
                    + transformer.getName().toLowerCase().replaceAll(" ", "_")
                    + ".name");
        }
        else
        {
    ...
    {
        if (itemstack.hasTagCompound()
                && itemstack.getTagCompound().hasKey("Items"))
        {
            NBTTagList nbttaglist = itemstack.getTagCompound().getTagList(
                    "Items", 10);
