...
public class ItemSkystrikesCrossbow extends ItemSword
{
            ...
            EntityPlayer player, int time)
    {
        ...
        {
            boolean flag = stack.hasTagCompound() ? stack.getTagCompound()
                    .getBoolean("blueMode") : false;
    ...
    public void onUsingTick(ItemStack stack, EntityPlayer player, int count)
    {
                ...
                && (player.inventory.hasItem(TFItems.energonCrystalPiece) || player.capabilities.isCreativeMode))
        {
            ...
            {
                float f = 1.0F;
                float f11 = player.prevRotationPitch
                        + (player.rotationPitch - player.prevRotationPitch) * f;
                float f21 = (player.prevRotationYaw + (player.rotationYaw - player.prevRotationYaw)
                        * f) + 90;
                double d01 = player.prevPosX + (player.posX - player.prevPosX)
                        * (double) f;
                double d11 = player.prevPosY
                        + (player.posY - player.prevPosY)
                        * (double) f
                        + (double) (player.worldObj.isRemote ? player
                                .getEyeHeight() - player.getDefaultEyeHeight()
                                : player.getEyeHeight());
                double d21 = player.prevPosZ + (player.posZ - player.prevPosZ)
                        * (double) f;
                ...
                for (int i = 0; i < 20; ++i)
                {
                    ...
                    {
                        boolean flag = stack.hasTagCompound() ? stack
                                .getTagCompound().getBoolean("blueMode")
                                : false;
                        player.worldObj.spawnParticle("reddust",
                                damageVec.xCoord, damageVec.yCoord
                                        - (isInVehichleMode ? 0.5D : 0.0D),
                                damageVec.zCoord, flag ? -1.0D : 0.0D, 0.0D,
                                flag ? 1.0D : 0.0D);
                    }

                    List<Entity> list = getEntitiesNear(player.worldObj,
                            damageVec.xCoord, damageVec.yCoord,
                            damageVec.zCoord, 0.3F);
