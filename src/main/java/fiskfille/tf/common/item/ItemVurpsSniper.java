...
public class ItemVurpsSniper extends ItemSword
{
            ...
            EntityPlayer player)
    {
        ...
        if (TFHelper.isPlayerVurp(player)
                ...
                && (player.inventory.hasItem(TFItems.miniMissile) || isCreativeMode))
        {
                ...
                if (TFShootManager.shotsLeft > 0)
                {
                    if (TFShootManager.shootCooldown <= 0)
                    {
                        TFPacketManager.networkWrapper
                                .sendToServer(new PacketVurpSniperShoot(player));
    ...
    {
        itemIcon = register.registerIcon(TransformersMod.modid + ":"
                + iconString);
    }
}