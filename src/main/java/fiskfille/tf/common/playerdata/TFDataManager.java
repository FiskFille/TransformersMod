...
public class TFDataManager
{
        ...
        if (vehicleMode != data.vehicle)
        {
            ...
            if (player.worldObj.isRemote)
            {
                TFPacketManager.networkWrapper
                        .sendToServer(new PacketHandleTransformation(player,
                                vehicleMode));
            }
            else
            {
                TFPacketManager.networkWrapper.sendToAllAround(
                        new PacketHandleTransformation(player, vehicleMode),
                        new TargetPoint(player.dimension, player.posX,
                                player.posY, player.posZ, 256));
            }

            data.vehicle = vehicleMode;

            MinecraftForge.EVENT_BUS.post(new PlayerTransformEvent(player,
                    vehicleMode, data.stealthForce));
        }
    }
    ...
    public static void setInStealthMode(EntityPlayer player, boolean stealthMode)
    {
        if (stealthMode != TFPlayerData.getData(player).stealthForce)
        {
            if (isInVehicleMode(player))
            {
                ...
                {
                    TFPacketManager.networkWrapper
                            .sendToServer(new PacketHandleStealthTransformation(
                                    player, stealthMode));
                }
                else
                {
                    TFPacketManager.networkWrapper.sendToAllAround(
                            new PacketHandleStealthTransformation(player,
                                    stealthMode), new TargetPoint(
    ...
    {
        return TFPlayerData.getData(player).vehicle
                && TFHelper.isPlayerTransformer(player);
    }
    ...
    public static boolean isInStealthMode(EntityPlayer player)
    {
        ...
        if (transformer != null)
        {
            return transformer.hasStealthForce(player)
                    && isInVehicleMode(player)
                    && TFPlayerData.getData(player).stealthForce;
        }
    ...
    public static void updateTransformationStatesFor(EntityPlayer player)
    {
        ...
        for (Object obj : player.worldObj.playerEntities)
        {
            if (obj instanceof EntityPlayer)
            {
                ...
                if (data != null)
                {
                    states.put(cPlayer.getUniqueID(), new Boolean[] {
                            data.vehicle, data.stealthForce });
                }
            }
        }

        TFPacketManager.networkWrapper.sendTo(new PacketSyncStates(states),
