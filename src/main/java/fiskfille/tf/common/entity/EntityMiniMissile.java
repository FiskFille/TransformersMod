...
public class EntityMiniMissile extends EntityArrow implements IProjectile
{
    ...
    public EntityMiniMissile(World world, EntityLivingBase shooter,
    ...
    public EntityMiniMissile(World world, EntityLivingBase shooter,
            float strength, boolean allowMissileExplosions)
    {
        super(world);
        this.renderDistanceWeight = 10.0D;
        this.shootingEntity = shooter;
        ...
        dZ = Math.cos((double) this.rotationYaw * Math.PI / 180.0D)
                * (1.2D + rand.nextFloat());
        dX = -Math.sin((double) this.rotationYaw * Math.PI / 180.0D)
                * (1.2D + rand.nextFloat());
        ...
        this.setThrowableHeading(this.motionX, this.motionY, this.motionZ,
                strength * 1.5F, shooter.rotationPitch);
     ...
     * direction.
     */
    public void setThrowableHeading(double x, double y, double z, float p1,
            float p2)
    {
    ...
    public void setPositionAndRotation2(double x, double y, double z,
            float yaw, float pitch, int p_70056_9_)
    {
        this.setPosition(x, y, z);
    ...
    public void setVelocity(double xVel, double yVel, double zVel)
    {
        ...
        if (this.prevRotationPitch == 0.0F && this.prevRotationYaw == 0.0F)
        {
            ...
            this.prevRotationYaw = this.rotationYaw = (float) (Math.atan2(xVel,
                    zVel) * 180.0D / Math.PI);
            this.prevRotationPitch = this.rotationPitch = (float) (Math.atan2(
                    yVel, (double) f) * 180.0D / Math.PI);
    ...
    public void onUpdate()
    {
        ...
        Block block = this.worldObj.getBlock(this.blockX, this.blockY,
                this.blockZ);

        if (block.getMaterial() != Material.air)
        {
            block.setBlockBoundsBasedOnState(this.worldObj, this.blockX,
                    this.blockY, this.blockZ);
        ...
        if (this.inGround)
        {
            ...
            this.motionY = 0;
            this.motionZ = 0;
            worldObj.createExplosion(this.shootingEntity, blockX, blockY,
                    blockZ, 2, missileExplosions);
            this.setDead();
        }
        else
        {
            ...
            for (int i = 0; i < 10; ++i)
            {
                ...
                worldObj.spawnParticle("smoke", posX + (rand.nextFloat() / 4),
                        posY + (rand.nextFloat() / 4), posZ
                                + (rand.nextFloat() / 4), 0, 0, 0);
            }

            Vec3 vecPos = Vec3.createVectorHelper(this.posX, this.posY,
                    this.posZ);
            ...
            MovingObjectPosition movingobjectposition = this.worldObj
                    .func_147447_a(vecPos, movementVec, false, true, false);
            vecPos = Vec3.createVectorHelper(this.posX, this.posY, this.posZ);
            movementVec = Vec3.createVectorHelper(this.posX + this.motionX,
                    this.posY + this.motionY, this.posZ + this.motionZ);

            if (movingobjectposition != null)
            {
                movementVec = Vec3.createVectorHelper(
            ...
            for (i = 0; i < entities.size(); ++i)
            {
                        ...
                        && (entity1 != this.shootingEntity || this.ticksInAir >= 5))
                {
                    ...
                    MovingObjectPosition movingobjectposition1 = axisalignedbb1
                            .calculateIntercept(vecPos, movementVec);
            ...
            if (movingobjectposition != null && entityHit != null
            ...
            if (movingobjectposition != null)
            {
                if (entityHit != null && entityHit != shootingEntity)
                {
                    ...
                    {
                        worldObj.createExplosion(shootingEntity,
                                entityHit.posX, entityHit.posY, entityHit.posZ,
                                2, missileExplosions);
                        this.setDead();
                    }
                    else
                ...
                else
                {
                    ...
                    {
                        hitBlock.onEntityCollidedWithBlock(this.worldObj,
                                this.blockX, this.blockY, this.blockZ, this);
                    }
                }
            }
