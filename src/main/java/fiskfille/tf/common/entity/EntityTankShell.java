...
public class EntityTankShell extends EntityArrow implements IProjectile
{
    ...
    public EntityTankShell(World world, EntityLivingBase shooter,
    ...
    public void onUpdate()
    {
        ...
        if (this.inGround)
        {
            int j = this.worldObj.getBlockMetadata(this.blockX, this.blockY,
                    this.blockZ);
            ...
            if (block == this.hitBlock && j == this.inData)
            {
                ++this.ticksInGround;
                this.setDead();
                this.worldObj.createExplosion(this.shootingEntity, posX, posY,
                        posZ, 1.0F, TFConfig.allowTankShellExplosions);
            }
            else
            {
                this.inGround = false;
        ...
        else
        {
            ...
            if (movingobjectposition != null)
            {
                if (entityHit != null && entityHit != shootingEntity)
                {
                    ...
                    this.worldObj
                            .createExplosion(this.shootingEntity, posX, posY,
                                    posZ, 1.0F,
                                    TFConfig.allowTankShellExplosions);
                    ...
                    if (entityHit.attackEntityFrom(damagesource, (float) k))
                    {
                        if (entityHit instanceof EntityLivingBase)
                        {
                            ...
                            {
                                f4 = MathHelper.sqrt_double(this.motionX
                ...
                else
                {
                    ...
                    this.hitBlock = this.worldObj.getBlock(this.blockX,
                            this.blockY, this.blockZ);
                    this.inData = this.worldObj.getBlockMetadata(this.blockX,
                            this.blockY, this.blockZ);
                    ...
                    {
                        this.hitBlock.onEntityCollidedWithBlock(this.worldObj,
    ...
    /**
     * Whether the tankshell has a stream of critical hit particles flying
     * behind it.
     */
