...
public class PacketCloudtrapJetpack implements IMessage
{
    ...
    public static class Handler implements
            IMessageHandler<PacketCloudtrapJetpack, IMessage>
    {
        public IMessage onMessage(PacketCloudtrapJetpack message,
                MessageContext ctx)
        {
            if (ctx.side.isClient())
            {
                ...
                {
                    ClientTickHandler.cloudtrapJetpacking.put(from,
                            message.jetpacking);
                }
            }
            else
            {
                ...
                TFPacketManager.networkWrapper.sendToAllAround(
                        new PacketCloudtrapJetpack(player, message.jetpacking),
                        new TargetPoint(player.dimension, player.posX,
                                player.posY, player.posZ, 64));
            }
