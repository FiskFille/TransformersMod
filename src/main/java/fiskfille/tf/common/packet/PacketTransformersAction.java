...
public class PacketTransformersAction implements IMessage
{
    ...
    public PacketTransformersAction(EntityPlayer player,
            PlayerInteractEvent.Action a)
    {
    ...
    public static class Handler implements
            IMessageHandler<PacketTransformersAction, IMessage>
    {
        public IMessage onMessage(PacketTransformersAction message,
                MessageContext ctx)
        {
            if (ctx.side.isClient())
            {
                ...
                if (fromEntity instanceof EntityPlayer)
                {
                    ...
                    if (message.action == Action.RIGHT_CLICK_AIR
                            || message.action == Action.RIGHT_CLICK_BLOCK)
                    {
                        ...
                        if (transformer != null)
                        {
                            ...
                            if (shootSound != null)
                            {
                                from.worldObj.playSound(from.posX, from.posY
                                        ...
                                        shootSound,
                                        transformer.getShootVolume(), 1, false);
                            }
                        }
                    }
                }
            ...
            else
            {
                ...
                if (from != null)
                {
                    if (message.action == Action.RIGHT_CLICK_AIR
                            || message.action == Action.RIGHT_CLICK_BLOCK)
                    {
                        Transformer transformer = TFHelper
                                .getTransformer(player);

                        if (transformer != null)
                        {
                            if (transformer.canShoot(player)
                                    && TFDataManager.isInVehicleMode(from))
                            {
                                Item shootItem = transformer.getShootItem();
                                boolean isCreative = from.capabilities.isCreativeMode;
                                boolean hasAmmo = isCreative
                                        || from.inventory.hasItem(shootItem);

                                if (hasAmmo)
                                {
                                    World world = from.worldObj;
                                    TFPacketManager.networkWrapper
                                            .sendToAll(new PacketTransformersAction(
                                                    player, message.action));

                                    Entity entity = transformer
                                            .getShootEntity(player);
                                    entity.posY--;
                                    world.spawnEntityInWorld(entity);

                                    if (!isCreative)
                                        player.inventory
                                                .consumeInventoryItem(shootItem);
                                }
                            }
                        }
                    }
